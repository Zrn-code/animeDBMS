...
function InternalPage(){
    ...
    const deleteRating = (anime_id) => async () => {
        try {
            const response = await axiosInstance.delete(`/api/removeRating`,{"anime_id":anime_id}, {
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `${token}`,
                },
                ...
                <div className='w-1/3 p-5'>
                    <TitleCard title="Rating" className="overflow-x-auto w-full">
                        <table className="table w-full">
                            ...
                            <tbody>
                            { Rating && Rating.slice(startIndexRating,endIndexRating).map((rating, index) => {
                            return(
                                <tr>
                                    <td><img src={rating.Image_URL} alt={rating.title}></img></td>
                                    <td>{rating.title}</td>
                                    <td>{rating.score} <TrashIcon className='w-8' onClick={deleteRating(rating.anime_id)} /> </td>
                                </tr>
                            )})}
                            </tbody>
                        </table>
                        ...
                        <button className="btn" 
                                onClick={()=>{if(RatingPage > 1) setRatingPage(RatingPage-1)}}
                                disabled={RatingPage === 1}
                        >
                          prev
                        </button>
                        <button className="btn" 
                                onClick={()=>{if(RatingPage < totalRatingPages) setRatingPage(RatingPage+1)}}
                ...
                <div className='w-1/3 p-5'>
                    <TitleCard title="Watch List">
                        <table className="table w-full">
                            ...
                            <tbody>
                            { WatchList && WatchList.slice(startIndexWatchList,endIndexWatchList).map((watchlist, index) => {
                            return(
                                <tr>
                                    <td><img src={watchlist.Image_URL} alt={watchlist.title}></img></td>
                        ...
                        <div className="flex justify-center btn-group">
                        <button className="btn" 
                                onClick={()=>{if(WatchListPage > 1) setWatchListPage(WatchListPage-1)}}
                                disabled={WatchListPage === 1}
                        >
                          prev
                        </button>
                        <button className="btn" 
                                onClick={()=>{if(WatchListPage < totalWatchListPages) setWatchListPage(WatchListPage+1)}}
